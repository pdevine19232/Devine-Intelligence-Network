...
function TaskDetail({ taskId, token, onClose, onApprove, onReject }) {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      if (data.previewing) {
        ...
        setPreviewError(data.error || "No frontend files to preview.");
      }
    } catch (e) {
  ...
  const handleRestorePreview = async () => {
    try {
      ...
      console.error("Restore preview failed:", e);
    }
  };
  ...
  const handleReject = async () => {
    ...
    if (previewing) await handleRestorePreview();
    try {
  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: "100%" }}>
      ...
      {canReview && (
        <div style={{
          background: previewing ? "#1a3a1a" : COLORS.blue,
          padding: "12px 28px",
          display: "flex",
          ...
          {previewing ? (
            ...
            <>
              <div style={{ fontSize: 13, color: "#c8d8e8" }}>
                Want to see the changes before deciding?
              </div>
              <button
              ...
              >
                {previewLoading ? "APPLYING..." : "PREVIEW IN BROWSER →"}
              </button>
            </>
          )}
        </div>
      ...
      {previewError && (
        <div style={{ padding: "10px 28px", background: "#fff4f4", fontSize: 13, color: COLORS.red }}>
          {previewError}
        </div>
      )}
