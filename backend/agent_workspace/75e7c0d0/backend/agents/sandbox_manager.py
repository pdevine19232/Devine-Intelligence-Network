...
def preview_frontend_changes(task_id: str, project_root: str = None) -> dict:
    ...
    if task_id in _running_previews:
        try:
            proc = _running_previews[task_id].get("frontend")
            if proc:
                proc.terminate()
                proc.wait(timeout=5)
        except Exception:
    ...
    try:
        # Create preview dir
        preview_dir = workspace_dir / "frontend_preview"
        ...
        shutil.copytree(
            ...
            preview_dir,
            dirs_exist_ok=True,
            ignore=shutil.ignore_patterns("node_modules", ".next", "dist", ".env")
        )

        # Apply Builder's frontend changes
        for builder_file in workspace_dir.rglob("*"):
            if builder_file.is_dir():
                continue
            rel = builder_file.relative_to(workspace_dir)
            ...
            if parts[0] not in ("frontend", "src"):
                continue

            # Skip meta files
            if parts[0] == "frontend" and parts[1] in ("node_modules", ".next", "dist"):
                continue
            ...
            dest = preview_dir / dest_rel
            dest.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(builder_file, dest)

        # Start dev server
        log_path = workspace_dir / "preview.log"
        log_file = open(log_path, "w", encoding="utf-8")

        # Try npm start first, fall back to yarn
        process = subprocess.Popen(
            ["npm", "start"],
            cwd=str(preview_dir),
            stdout=log_file,
            stderr=log_file,
            env={**os.environ, "PORT": str(PREVIEW_FRONTEND_PORT)},
        )

        _running_previews[task_id] = {"frontend": process}
        print(f"[Preview] Started frontend on port {PREVIEW_FRONTEND_PORT}")
